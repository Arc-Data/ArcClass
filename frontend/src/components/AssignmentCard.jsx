...
const AssignmentCard = ({ post }) => {
    return (
        <Link to={`/assignments/${post.assignment.id}`} className="block w-full gap-4 border rounded-lg shadow-sm cursor-pointer group hover:bg-gray-200">
            <div className="flex gap-4 px-8 py-4">
                ...
                <div className="flex-1 space-y-4">
                    <div className="flex items-center justify-between">
                        ...
                        <p className="p-1 px-4 border rounded-lg shadow-sm group-hover:bg-primary group-hover:text-white">{getDeadline(post.assignment.submissionDate)}</p>
                    </div>
